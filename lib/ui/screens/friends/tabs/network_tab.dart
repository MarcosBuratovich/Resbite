...
class NetworkTab extends ConsumerWidget {
  ...
  final Function(dynamic, {required bool isResbiteUser, bool isPending}) buildContactItem;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final contactPermissionAsync = contactService.hasContactsPermission();

    // Watch direct friends and pending friend requests for UI state
    final directFriendsAsync = ref.watch(friends_services.directFriendsListProvider);
    final pendingRequestsAsync = ref.watch(friends_services.pendingFriendRequestsProvider);
          ...
          return contactsAsync.when(
            data: (contacts) {
              ...
              return networkAsync.when(
                data: (network) {
                  // Filter out contacts who are already direct friends and annotate pending
                  final directFriendIds = directFriendsAsync.asData?.value.map((f) => f.user.id).toSet() ?? {};
                  final pendingFriendIds = pendingRequestsAsync.asData?.value.map((r) => r.requester.id).toSet() ?? {};
                  final resbiteUsers = filteredContacts
                      .where((contact) => contact.isResbiteUser == true)
                      .where((contact) => contact.resbiteUserId != null && !directFriendIds.contains(contact.resbiteUserId!))
                      .toList();

                  final nonResbiteUsers = filteredContacts.where((contact) => contact.isResbiteUser == false).toList();

                  // If no contacts or network, show prompt
                  ...
                  return ListView(
                    ...
                    children: [
                      ...
                      if (resbiteUsers.isNotEmpty) ...[
                        ...
                        ...resbiteUsers.map(
                          (contact) => buildContactItem(
                            contact,
                            isResbiteUser: true,
                            isPending: pendingFriendIds.contains(contact.resbiteUserId!),
                          ),
                        ),
                        const SizedBox(height: 24),
                      ],
                      ...
                      const SizedBox(height: 24),
                      // Extended peers (friends of friends)
                      if (network.isNotEmpty) ...[
                        ...
                        ...network.map(
                          (conn) => buildNetworkConnectionItem(conn),
                        ),
                      ],
                    ],
                  );
